...
def run_discovery_background(job_id, user_id, seed_type, access_token,
    ...
    try:
        ...
        if seed_type == "liked":
            ...
            # "More like this" seed — single artist name
        ...
        set_progress(job_id, 30, "Finding similar artists via Last.fm...")
        ...
        for artist_name in list(seen_artists.keys())[:target_artists]:
            tracks = search_artist_tracks(sp, artist_name, count=3)
            found.extend(tracks)
...
def start():
    ...
    title = custom_name if custom_name else f"Rabbithole - {seed_name}"
    ...
    with get_conn() as conn:
        ...
        row2 = conn.execute(
            text("""SELECT u.access_token, p.excluded_genres, p.playlist_size,
                           p.adventurousness, p.genre_spread
...
def reroll(playlist_id):
    ...
    seed_type, seed_name, excl, psize, adv, spread = row
